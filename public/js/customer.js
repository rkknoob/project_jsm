$(document).ready(function () {
  ...
  registerForm.on('submit', function (e) {
    ...
    axios.post($base_url + '/users', data).then(function (response) {
      ...
      showFormErrors(error.response, registerForm);
    });
  });
  customerForm.on('submit', function (e) {
    e.preventDefault();
    var data = customerForm.serialize();
    axios.put($base_url + '/users/' + $('input[name=user_id]').val(), data).then(function (response) {
      if (!response.data.status) {
        Swal({
          type: 'error',
          title: response.data.title,
      ...
      window.location.href = $base_url + '/users/me';
    })["catch"](function (error) {
      showFormErrors(error.response, customerForm);
    });
  }); // event province
